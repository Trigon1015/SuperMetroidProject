    ...
    private void Update()
    {
        ...
        if (!battleEnded)
        {
            ...
            else
            {
                ...
                {
                    targetPoints = theBoss;
                    fadeCounter = fadeoutTime;
                    anim.SetTrigger("vanish");
                }
                else
                {
                    if (Vector3.Distance(theBoss.position, targetPoints.position) > 0.2f)
                    {
                        theBoss.position = Vector3.MoveTowards(theBoss.position, targetPoints.position, moveSpeed * Time.deltaTime);
                    ...
                    else if (inactiveCounter > 0)
                    {
                        ...
                        if (inactiveCounter <= 0)
                        {
                            ...
                            targetPoints = spawnPoints[Random.Range(0, spawnPoints.Length)];
                            int whileBreaker = 0;
                            while (targetPoints.position == theBoss.position && whileBreaker < 100)
                            {
                                targetPoints = spawnPoints[Random.Range(0, spawnPoints.Length)];
                                whileBreaker++;
                            }
